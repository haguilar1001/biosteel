...
async function main() {
  ...
  const marcaIpsPorAnio = new Map<number, FilaMarcaIpsAgg[]>();
  ...
  for (const anio of anios) {
    ...
    const marcaIps = marcaIpsPorAnio.get(anio) ?? [];
    if (!DRY) {
      ...
      await crearEnLotes(marcaIps, (c) => prisma.ventaMarcaIps.createMany({ data: c.map((e) => ({ anio: e.anio, mes: e.mes, marca: e.marca, ips: e.ips, valor: dec(e.valor), costo: dec(e.costo) })) }));
    }
    console.log(`   ${DRY ? "•" : "✅"} ${anio}: venta neta $ ${fmt(netoPorAnio.get(anio) ?? 0)} (${lineas.length} líneas, ${clientes.length} clientes, ${marcas.length} marcas, ${marcaIps.length} marca×ips)`);
  }
